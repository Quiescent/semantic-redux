...
(defvar mode-variable-flag)
(defvar constant-flag)
(defvar override-flag)
...
(defun mode-local-on-major-mode-change ()
  ...
  (when (not (member (current-buffer) mode-local-changed-mode-buffers))
    (push (current-buffer) mode-local-changed-mode-buffers))
